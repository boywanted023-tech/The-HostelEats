  ...
  const Api = {
    ...
    async request(path, options = {}) {
      ...
      if (!['GET', 'HEAD'].includes(method)) {
        const { ts } = window.Security.signRequest(options.body || {});
        headers['X-Request-Timestamp'] = ts;
      }

      const config = { method, headers, credentials: 'include' };
      if (options.body && method !== 'GET') config.body = JSON.stringify(window.Security.sanitizeObject(options.body));
    ...
    customer: {
      vendors: (params) => Api.get('/customer/vendors' + (params ? '?' + new URLSearchParams(params) : '')),
      vendor: (id) => Api.get('/customer/vendors/' + id),
      menu: (vendorId) => Api.get('/customer/menu/' + vendorId),
      categories: () => Api.get('/customer/categories'),
      placeOrder: (data) => Api.post('/customer/orders', data),
      orders: () => Api.get('/customer/orders'),
      order: (id) => Api.get('/customer/orders/' + id),
      cancelOrder: (id, reason) => Api.post('/customer/orders/' + id + '/cancel', { reason }),
      rateOrder: (id, data) => Api.post('/customer/orders/' + id + '/rate', data)
    }
  };
